...
public class TutorialArrowHandle : MonoBehaviour
{
    [SerializeField]
    private Image _arrowImage = null;

    private TutorialMoveTarget _target = null;
    ...
    public void SetTarget(TutorialMoveTarget target)
    {
    ...
    private IEnumerator ArrowUpdateCoroutine()
    {
        ...
        {
            Vector3 targetScreenPos = _mainCamera.WorldToScreenPoint(_target.transform.position);
            ...
            Vector3 viewportPosition = _mainCamera.WorldToViewportPoint(_target.transform.position);
